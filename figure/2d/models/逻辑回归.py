...
# 集成模型
lr =  LogisticRegression(random_state = 256)
# 模型训练
lr.fit(X_train, y_train)
# 评价指标
expected = y_test.astype('int')
precited = lr.predict(X_test).astype('int')
# AUC,ROC曲线下面积
rf_auc = metrics.roc_auc_score(y_test,lr.predict_proba(X_test)[:,1])
...
if not os.path.exists('logs'):
    os.mkdir('logs')
dir_name='逻辑回归'+str(time.strftime("%Y%m%d-%H-%M", time.localtime()))+'smote_34'
...
# 绘制ROC曲线
draw_roc(y_test, lr.predict_proba(X_test)[:,1],'LR', os.path.join(path, './test_roc.pdf'))
