...
precited = svm.predict(X_test).astype('int')
# AUC,ROC曲线下面积
rf_auc = metrics.roc_auc_score(y_test,svm.predict_proba(X_test)[:,1])
...
if not os.path.exists('logs'):
    os.mkdir('logs')
dir_name='SVM'+str(time.strftime("%Y%m%d-%H-%M", time.localtime()))+'smote_34'
...
# 绘制ROC曲线
draw_roc(y_test, svm.predict_proba(X_test)[:,1],'SVM', os.path.join(path, './test_roc.pdf'))
