...
# 集成模型
rf = RandomForestClassifier(max_depth=100,min_samples_split=2,max_features = 100, random_state = 256)
# 模型训练
rf.fit(X_train, y_train)
# 评价指标
expected = y_test.astype('int')
precited = rf.predict(X_test).astype('int')
# AUC,ROC曲线下面积
...
if not os.path.exists('logs'):
    os.mkdir('logs')
dir_name='随机森林'+str(time.strftime("%Y%m%d-%H-%M", time.localtime()))+'smote_34'
...
# 绘制ROC曲线
draw_roc(y_test, rf.predict_proba(X_test)[:,1],'RF', os.path.join(path, './test_roc.pdf'))
