...
# 集成模型
sgd = SGDClassifier(loss='modified_huber',max_iter=1000, random_state = 256)
# 模型训练
sgd.fit(X_train, y_train)
# 评价指标
expected = y_test.astype('int')
precited = sgd.predict(X_test).astype('int')
# AUC,ROC曲线下面积
rf_auc = metrics.roc_auc_score(y_test,sgd.predict_proba(X_test)[:,1])
...
if not os.path.exists('logs'):
    os.mkdir('logs')
dir_name='SGD'+str(time.strftime("%Y%m%d-%H-%M", time.localtime()))+'smote_34'
...
# 绘制ROC曲线
draw_roc(y_test, sgd.predict_proba(X_test)[:,1],'SGD', os.path.join(path, './test_roc.pdf'))
