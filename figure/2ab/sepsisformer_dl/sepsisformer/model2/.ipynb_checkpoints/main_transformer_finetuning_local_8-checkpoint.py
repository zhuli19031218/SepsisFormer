...
def hyperParameters():
    ...
    parser.add_argument('--lr', type=float, default=5e-4,
                        help='初始学习率。')
    parser.add_argument('--epoch', type=int, default=2600, # 可根据需要调整轮数
                        help='训练的总轮数。')
    parser.add_argument('--batch_size', type=int, default=5000,
                        help='数据加载的批量大小。')
    parser.add_argument('--pretrain',  default=False, # 使用 action='True'加载   False 不加载 
                        help='加载预训练权重 (不包括 MLP 头)。设置此项以启用。')
    parser.add_argument('--loadmodel', type=str, default= "/mnt/public/home/zhijiangwan/sepsisformer/sepsisformer/model2/logs/Transformer_426_pretrain/best_acc_model.pth",help='预训练模型文件 (.pth) 的路径。如果设置了 --pretrain，则此项为必需。')
