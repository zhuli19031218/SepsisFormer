...
def hyperParameters():
    ...
    parser.add_argument('--data_path', type=str,
                        default="/mnt/public/home/zhijiangwan/sepsisformer/sepsisformer/data/8/local_smote_8.csv",
                        help="数据集 CSV 文件的路径。")
    ...
    parser.add_argument('--lr', type=float, default=2e-4,
                        help='初始学习率。')
    parser.add_argument('--epoch', type=int, default=1500, # 可根据需要调整轮数
                        help='训练的总轮数。')
    parser.add_argument('--batch_size', type=int, default=3762,
                        help='数据加载的批量大小。(!!!如果显存不足请减小!!!)')
    parser.add_argument('--seed', type=int, default=42,
                        help='设置随机种子以保证结果可复现。')

    
    # --- 新增：预训练相关参数 --- 
    parser.add_argument('--pretrain',  default=True, # 使用 action='True'  False 使其成为一个标志
                        help='加载预训练权重 (不包括 MLP 头)。设置此项以启用。')
    parser.add_argument('--loadmodel', type=str, default= "/mnt/public/home/zhijiangwan/sepsisformer/sepsisformer/model2/logs/GRU_pretrain/best_acc_model.pth",help='预训练模型文件 (.pth) 的路径。如果设置了 --pretrain，则此项为必需。')
