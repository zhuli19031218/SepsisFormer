...
def SVM_classifier(train_features, train_labels, test_features, test_labels, c, kernel):
    ...
    model = sklearn.svm.SVC(C=10, kernel= 'linear' , degree=10)  # (适用于样例数量少于10000时的二元和多元分类）
    #model = joblib.load(r'F:\sepsisformer\model2\results\SVM.model')
...
def main():
    ...
    dir_name = ml_get_dir_name(model_name=f'Support Vector Machine_{kernel}2')
    ...
    standard_pyh_test_inputs, pyh_test_labels = ml_get_data(path=r'F:\sepsisformer\data\8\mimic4_smote_8.csv',
