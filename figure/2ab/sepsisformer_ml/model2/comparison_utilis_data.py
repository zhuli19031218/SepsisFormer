...
from sklearn.model_selection import train_test_split # 推荐使用 train_test_split 进行划分
from sklearn.preprocessing import StandardScaler # 假设 stander_data 内部逻辑
...
def ml_get_data(path: str, logs, stage, test_size=0.6): # test_size = 1.0 - 0.4，与你之前的比例一致
    np.random.seed(256)
    df = pd.read_csv(path)

    # 确保 'dead' 列存在
    if 'dead' not in df.columns:
        raise ValueError(f"在文件 {path} 中未找到标签列 'dead'")

    # 1. 首先分离特征 (X) 和标签 (y)
    X = df.drop(columns=['dead'])
    y = df['dead']

    # (可选) 行打乱，train_test_split 也可以实现
    # X = X.sample(frac=1, random_state=256).reset_index(drop=True)
    # y = y.loc[X.index] # 保持 y 与 X 的行对应

    # 2. 划分训练集和测试集
    # 使用 train_test_split 更标准，它能确保 X 和 y 的划分是一致的，还可以分层抽样
    X_train, X_test, y_train, y_test = train_test_split(
        X, y,
        test_size=test_size,       # 测试集比例
        random_state=256,          # 随机种子保证可复现
        stratify=y                 # 对于分类问题，使用分层抽样保持标签比例很重要
    )

    # --- 特征标准化步骤 ---
    # 3. 初始化标准化器
    scaler = StandardScaler() # 使用 scikit-learn 的标准化器

    # 4. 在 *训练集特征* 上拟合 (fit) 标准化器
    scaler.fit(X_train)

    # 5. 使用拟合好的标准化器转换 (transform) 训练集和测试集的特征
    X_train_scaled = scaler.transform(X_train)
    X_test_scaled = scaler.transform(X_test)

    # （可选）如果后续需要 DataFrame 格式，可以转换回去，并保留列名
    # X_train_scaled_df = pd.DataFrame(X_train_scaled, columns=X_train.columns, index=X_train.index)
    # X_test_scaled_df = pd.DataFrame(X_test_scaled, columns=X_test.columns, index=X_test.index)
    # --- 标准化结束 ---

    # 6. 将标签转换为 NumPy 数组 (它们之前已经被分离出来了)
    y_train_np = y_train.to_numpy()
    y_test_np = y_test.to_numpy()

    # 打印形状检查 (可选)
    print('标准化后训练特征形状:', X_train_scaled.shape)
    print('训练标签形状:', y_train_np.shape)
    print('标准化后测试特征形状:', X_test_scaled.shape)
    print('测试标签形状:', y_test_np.shape)

    # 7. 返回标准化后的特征 和 对应的标签
    # 注意返回顺序，与你之前的函数保持一致
    return X_train_scaled, y_train_np, X_test_scaled, y_test_np
